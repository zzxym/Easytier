...
m:section(SimpleSection).template  = "sdwan/sdwan_status"

-- sdwan-core
s=m:section(TypedSection, "sdwan", translate("SDWAN Configuration"))
s.addremove=false
s.anonymous=true
...
btncq.write = function()
  luci.sys.call("/etc/init.d/sdwan restart >/dev/null 2>&1 &")  -- 执行重启命令
end
...
et_config = s:taboption("general", TextValue, "et_config", translate("Configuration File"),
        translate("The configuration file is located at /etc/sdwan/config.toml<br>"
...
et_config.cfgvalue = function(self, section)
    return nixio.fs.readfile("/etc/sdwan/config.toml") or ""
end
...
ip_dhcp = s:taboption("general", Flag, "ip_dhcp", translate("Enable DHCP"),
        translate("IP address will be automatically determined and assigned by SDWAN, starting from 10.0.0.1 by default. "
...
peeradd.placeholder = "tcp://public.sdwan.top:11010"
peeradd:value("tcp://public.sdwan.top:11010", translate("Official Server - tcp://public.sdwan.top:11010"))
...
external_node.placeholder = "tcp://public.sdwan.top:11010"
external_node:value("tcp://public.sdwan.top:11010", translate("Official Server - tcp://public.sdwan.top:11010"))
...
uuid.cfgvalue = function(self, section)
    return nixio.fs.readfile("/etc/sdwan/et_machine_id") or ""
end
uuid.write = function(self, section, value)
    nixio.fs.writefile("/etc/sdwan/et_machine_id", value:gsub("\r\n", "\n"))
end
...
accept_dns = s:taboption("privacy", Flag, "accept_dns", translate("Enable Magic DNS"),
        translate("With Magic DNS, you can access other nodes using domain names, e.g., <hostname>.sdwan.net. "
...
et_forward:value("etfwlan", translate("Allow traffic from SDWAN virtual network to LAN"))
et_forward:value("etfwwan", translate("Allow traffic from SDWAN virtual network to WAN"))
et_forward:value("lanfwet", translate("Allow traffic from LAN to SDWAN virtual network"))
et_forward:value("wanfwet", translate("Allow traffic from WAN to SDWAN virtual network"))
...
check = s:taboption("privacy", Flag, "check", translate("Connectivity Check"),
        translate("Enable connectivity check to specify remote device IPs; if all specified IPs fail to ping, "
                .. "the SDWAN program will restart."))
...
local process_status = luci.sys.exec("ps | grep sdwan-core| grep -v grep")
...
btn1info.cfgvalue = function(self, section)
    local content = nixio.fs.readfile("/tmp/sdwan-cli_peer") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end
...
btn2.write = function()
    if process_status ~= "" then
        luci.sys.call("$(dirname $(uci -q get sdwan.@sdwan[0].sdwanbin))/sdwan-cli connector >/tmp/sdwan-cli_connector 2>&1")
    else
        luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/sdwan-cli_connector")
    end
end
...
btn2info.cfgvalue = function(self, section)
    local content = nixio.fs.readfile("/tmp/sdwan-cli_connector") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end
...
btn3.write = function()
    if process_status ~= "" then
        luci.sys.call("$(dirname $(uci -q get sdwan.@sdwan[0].sdwanbin))/sdwan-cli stun >/tmp/sdwan-cli_stun 2>&1")
    else
        luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/sdwan-cli_stun")
    end
end
...
btn3info.cfgvalue = function(self, section)
    local content = nixio.fs.readfile("/tmp/sdwan-cli_stun") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end
...
btn4.write = function()
    if process_status ~= "" then
        luci.sys.call("$(dirname $(uci -q get sdwan.@sdwan[0].sdwanbin))/sdwan-cli route >/tmp/sdwan-cli_route 2>&1")
    else
        luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/sdwan-cli_route")
    end
end
...
btn4info.cfgvalue = function(self, section)
    local content = nixio.fs.readfile("/tmp/sdwan-cli_route") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end
...
btnrm.write = function()
  os.execute("rm -rf /tmp/sdwan*.tag /tmp/sdwan*.newtag /tmp/sdwan-core_*")
end


easytierbin = s:taboption("upload", Value, "easytierbin", translate("sdwan-core Binary Path"),
        translate("Customize the storage path for sdwan-core. Make sure to provide the full path and filename. "
                .. "If the specified path has insufficient space, it will automatically move to /tmp/sdwan-core"))
easytierbin.placeholder = "/usr/bin/sdwan-core"
easytierbin.default = "/usr/bin/sdwan-core"

webbin = s:taboption("upload", Value, "webbin", translate("sdwan-web Binary Path"),
        translate("Customize the storage path for sdwan-web. Make sure to provide the full path and filename, "
                .. "then upload the installer"))
webbin.placeholder = "/usr/bin/sdwan-web"
webbin.default = "/usr/bin/sdwan-web"

...
pub fn get_inst_config(
    ...
    config.set_listeners(vec![
        "tcp://0.0.0.0:10010".parse().unwrap(),
        "udp://0.0.0.0:10010".parse().unwrap(),
        "wg://0.0.0.0:10011".parse().unwrap(),
        "ws://0.0.0.0:10011".parse().unwrap(),
    ...
    if proto == "tcp" {
        inst1
            ...
            .add_connector(TcpTunnelConnector::new(
                "tcp://10.1.1.2:10010".parse().unwrap(),
            ));
    } else if proto == "udp" {
        inst1
            .get_conn_manager()
            .add_connector(UdpTunnelConnector::new(
                "udp://10.1.1.2:10010".parse().unwrap(),
            ));
    } else if proto == "wg" {
        #[cfg(feature = "wireguard")]
        inst1
...
pub async fn proxy_three_node_disconnect_test(#[values("tcp", "wg")] proto: &str) {
    ...
    if proto == "tcp" {
        inst4
            .get_conn_manager()
            .add_connector(TcpTunnelConnector::new(
                "tcp://10.1.2.3:10010".parse().unwrap(),
            ));
    } else if proto == "wg" {
        inst4
            .get_conn_manager()
...
async fn avoid_tunnel_loop_back_to_virtual_network() {
    ...
    let tcp_connector = TcpTunnelConnector::new("tcp://10.144.144.2:10010".parse().unwrap());
    insts[0]
        .get_peer_manager()
    ...
    let udp_connector = UdpTunnelConnector::new("udp://10.144.144.3:10010".parse().unwrap());
    insts[0]
        .get_peer_manager()
